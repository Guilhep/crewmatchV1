-- =====================================================
-- SCRIPT DE CONFIGURAÇÃO - CREW MATCH (VERSÃO CORRIGIDA)
-- Sistema de Autenticação e Onboarding
-- Execute este script no SQL Editor do Supabase
-- Este script adiciona colunas mesmo se a tabela já existir
-- =====================================================

-- 1. Criar tabela profiles se não existir, ou adicionar colunas faltantes
...
BEGIN
  ...
  -- Adicionar main_skill se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'profiles' 
    AND column_name = 'main_skill'
  ) THEN
    ALTER TABLE public.profiles 
    ADD COLUMN main_skill TEXT;
    
    RAISE NOTICE 'Coluna main_skill adicionada';
  END IF;

  -- Adicionar level_id se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'profiles' 
    ...
    ALTER TABLE public.profiles 
    ADD COLUMN level_id TEXT CHECK (level_id IN ('silver', 'bronze', 'trainee'));
    
    RAISE NOTICE 'Coluna level_id adicionada';
  END IF;
  ...
  -- Adicionar skills se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'profiles' 
    ...
    RAISE NOTICE 'Coluna skills adicionada';
  END IF;

  -- Adicionar portfolio se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'profiles' 
    ...
    RAISE NOTICE 'Coluna portfolio adicionada';
  END IF;

  -- Garantir que role não pode ser null (se já tiver dados, atualizar primeiro)
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'profiles' 
    ...
    -- Atualizar valores null para 'professional'
    UPDATE public.profiles SET role = 'professional' WHERE role IS NULL;
    
    -- Tornar NOT NULL
    ALTER TABLE public.profiles 
    ALTER COLUMN role SET NOT NULL;
    
    RAISE NOTICE 'Coluna role agora é NOT NULL';
  END IF;

  -- Garantir que patent não pode ser null
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'profiles' 
    ...
    -- Atualizar valores null para 'bronze'
    UPDATE public.profiles SET patent = 'bronze' WHERE patent IS NULL;
    ...
    RAISE NOTICE 'Coluna patent agora é NOT NULL';
  END IF;
END $$;
...
-- 7. Criar função handle_new_user com SECURITY DEFINER
...
-- 9. Criar trigger que executa após inserção em auth.users
...
-- 13. Criar tabela de questões do quiz (opcional, para armazenar perguntas)
...
-- 14. Habilitar RLS na tabela quiz_questions
ALTER TABLE public.quiz_questions ENABLE ROW LEVEL SECURITY;

-- 15. Remover política antiga se existir
DROP POLICY IF EXISTS "Quiz questions are viewable by everyone" ON public.quiz_questions;

-- 16. Política: Todos podem ler questões do quiz
CREATE POLICY "Quiz questions are viewable by everyone"
  ON public.quiz_questions
  FOR SELECT
  USING (true);

-- 17. Criar tabela de respostas do quiz (opcional, para histórico)
...
-- 18. Habilitar RLS na tabela quiz_responses
ALTER TABLE public.quiz_responses ENABLE ROW LEVEL SECURITY;

-- 19. Remover políticas antigas se existirem
...
-- 20. Política: Usuários podem ver apenas suas próprias respostas
CREATE POLICY "Users can view own quiz responses"
  ON public.quiz_responses
  FOR SELECT
  USING (auth.uid() = user_id);

-- 21. Política: Usuários podem inserir suas próprias respostas
CREATE POLICY "Users can insert own quiz responses"
  ON public.quiz_responses
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- 22. Criar índices para quiz_responses
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'profiles' AND column_name = 'patent') THEN
    RAISE NOTICE '✓ Coluna patent existe na tabela profiles';
  ELSE
    RAISE WARNING '✗ Coluna patent não encontrada';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'profiles' AND column_name = 'quiz_score') THEN
    RAISE NOTICE '✓ Coluna quiz_score existe na tabela profiles';
  ELSE
