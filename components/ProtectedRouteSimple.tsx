...
/**
 * Versão simplificada do ProtectedRoute para uso com sistema de roteamento baseado em estado
 * (sem React Router)
 */
const ProtectedRouteSimple: React.FC<ProtectedRouteSimpleProps> = ({
  children,
  requiredRole,
  onUnauthorized,
  redirectTo,
  ...
  useEffect(() => {
    ...
    // Se não há usuário, redirecionar
    if (!user) {
      redirectTo?.();
      return;
    }
  ...
  // Se há role requerido e não corresponde, mostrar mensagem
  if (requiredRole && profile?.role !== requiredRole) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-navy">
